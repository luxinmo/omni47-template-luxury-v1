 ...
 * Confirmation email sent to the CLIENT after they request info
 * about a new development / promotion. Shows the project they
 * enquired about with current availability.
...
const st = {
  ...
  /* Hero image — clean, no dark overlay */
  heroImg: {
    width: "100%", height: 300, objectFit: "cover" as const, display: "block",
  } as React.CSSProperties,

  /* Project info bar — LIGHT version */
  infoBar: {
    backgroundColor: B.white, padding: "22px 36px",
    borderBottom: `1px solid ${B.border}`,
  } as React.CSSProperties,
  projectName: {
    fontSize: 22, fontWeight: 400, color: B.darkText, margin: "0 0 4px",
    letterSpacing: "0.04em",
  } as React.CSSProperties,
  projectLocation: {
    fontSize: 13, color: B.muted, margin: 0, letterSpacing: "0.03em",
  } as React.CSSProperties,
  ...
  statValue: {
    fontSize: 18, fontWeight: 600, color: B.darkText, margin: "0 0 2px",
  } as React.CSSProperties,
  statLabel: {
    fontSize: 10, textTransform: "uppercase" as const,
    letterSpacing: "0.1em", color: B.muted, margin: 0,
  } as React.CSSProperties,

  /* Content */
  content: {
    backgroundColor: B.white, padding: "32px 42px",
  } as React.CSSProperties,
  greeting: {
    fontSize: 22, fontWeight: 300, color: B.darkText, margin: "0 0 10px",
    letterSpacing: "0.02em",
  } as React.CSSProperties,
  intro: {
    fontSize: 14, lineHeight: 1.7, color: B.muted, margin: "0 0 24px",
  } as React.CSSProperties,
  ...
  progressWrap: { marginBottom: 24 } as React.CSSProperties,
  progressTrack: {
  ...
  table: {
    width: "100%", borderCollapse: "collapse" as const, marginBottom: 24,
  } as React.CSSProperties,
  th: {
      ...
      <div style={st.wrapper}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap" as const }}>
            ...
            {d.completionDate && (
              <div style={{ textAlign: "right" as const }}>
                <p style={{ fontSize: 10, color: B.muted, margin: "0 0 2px", textTransform: "uppercase" as const, letterSpacing: "0.1em" }}>Delivery</p>
                <p style={{ fontSize: 15, fontWeight: 500, color: B.darkText, margin: 0 }}>{d.completionDate}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div style={st.content} className="da-content">
          ...
          <p style={st.greeting} className="da-greeting">
            {d.recipientName ? `Thank you, ${d.recipientName}` : "Thank you for your interest"}
          </p>
          <p style={st.intro} className="da-intro">
            We've received your enquiry about <strong style={{ color: B.darkText }}>{d.projectName}</strong> in {d.projectLocation}.
            Below you'll find the current availability for this project.
            {keyReadyCount > 0 && (
              <> There {keyReadyCount === 1 ? "is" : "are"} currently <strong style={{ color: "hsl(38 70% 38%)" }}>{keyReadyCount} key-ready {keyReadyCount === 1 ? "unit" : "units"}</strong> available for immediate move-in.</>
            )}
            {" "}An advisor will be in touch with you shortly.
          </p>

          {/* Construction progress */}
          {d.constructionPercent != null && d.constructionPercent < 100 && (
          ...
          {d.contactName && (
            <div className="da-contact-box" style={{
              ...
              <p style={{ fontSize: 10, textTransform: "uppercase" as const, letterSpacing: "0.12em", color: B.muted, margin: "0 0 5px" }}>Your Advisor</p>
          ...
          {/* Note */}
          <p style={{ fontSize: 11, color: B.light, marginTop: 20, lineHeight: 1.6, textAlign: "center" as const }}>
            If you didn't submit this enquiry, please disregard this email.
          </p>
        </div>

        {/* ── C: Footer ── */}
        <div style={st.footer} className="da-footer">
          <p style={st.footerText}>Prestige Real Estate · Marbella</p>
          <p style={st.footerText}>© {new Date().getFullYear()} All rights reserved</p>
        </div>
      </div>
    </div>
  );
