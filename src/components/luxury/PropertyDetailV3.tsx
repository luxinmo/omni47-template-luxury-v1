...
import {
  ...
  Play, View, FileDown, Home, Shield, Sparkles, Clock, Grid3X3, Images,
} from "lucide-react";
...
const PropertyDetailV3 = () => {
  ...
  return (
    <div className="flex-1 overflow-auto bg-white text-luxury-black font-sans">
      ...
      {/* ═══ PHOTO MOSAIC (Airbnb / Bravos Estate style) ═══ */}
      <section className="relative">
        <div className="grid grid-cols-4 grid-rows-2 gap-1.5 h-[420px] md:h-[480px] lg:h-[540px]">
          ...
          <div className="relative overflow-hidden cursor-pointer group" onClick={() => setLightbox(1)}>
            <img src={p.images[1]} alt="" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
          </div>
          ...
          <div className="relative overflow-hidden cursor-pointer group" onClick={() => setLightbox(2)}>
            ...
            <div className="absolute top-3 right-3 flex items-center gap-2">
              ...
              <button
                ...
                className={`flex items-center gap-1.5 text-[12px] font-medium px-3.5 py-2 rounded-full shadow-sm transition-all ${liked ? "bg-luxury-black text-white" : "bg-white/90 backdrop-blur-sm text-luxury-black hover:bg-white"}`}
              >
          ...
          {/* Bottom-right 2 — with "Show all photos" */}
          <div className="relative overflow-hidden cursor-pointer group" onClick={() => setLightbox(4)}>
            ...
            <button
              ...
              className="absolute bottom-3 right-3 flex items-center gap-2 bg-white/90 backdrop-blur-sm text-luxury-black text-[13px] font-medium px-4 py-2.5 rounded-lg shadow-md hover:bg-white transition-all"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-8">
          <div className="lg:col-span-7">
            ...
            <p className="text-[13px] text-luxury-black/50 font-light mb-2">
              Detached houses <span className="mx-1 text-luxury-black/25">|</span> <span className="italic">{p.style}</span> <span className="mx-1 text-luxury-black/25">|</span> <span className="font-mono text-luxury-black/40 text-[12px]">REF-{p.ref}</span>
            </p>

            {/* Title */}
            <h1 className="text-[22px] md:text-[26px] lg:text-[30px] font-medium text-luxury-black leading-snug uppercase tracking-[0.01em] mb-2">
              {p.title}
            </h1>
            ...
            <div className="flex items-center gap-6 mt-5">
              {[
                ...
                <div key={i} className="flex items-center gap-2 text-[14px] text-luxury-black/70">
                  ...
                  <span className="text-[11px] text-luxury-black/40 uppercase tracking-wider">{s.label}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="lg:col-span-5">
            <div className="bg-neutral-50 border border-neutral-200 rounded-sm p-6">
                ...
                <div className="flex items-center gap-2">
                  <Popover>
                    <PopoverTrigger asChild>
                      <button className="w-8 h-8 flex items-center justify-center rounded-full border border-neutral-200 text-luxury-black/40 hover:text-luxury-black hover:border-neutral-400 transition-all">
              ...
              <div className="flex flex-wrap items-baseline gap-3 mb-2">
                <p className="text-[32px] font-extralight text-luxury-black tracking-tight leading-none">{p.price}</p>
              ...
              <div className="flex gap-2">
                ...
                <button className="flex-1 flex items-center justify-center gap-2 border border-neutral-300 text-luxury-black text-[12px] tracking-[0.1em] uppercase py-3 hover:bg-neutral-50 transition-all">
            ...
            {SIMILAR.map((s, i) => (
              <a key={i} href="#" className="group grid grid-cols-1 md:grid-cols-12 gap-0 bg-neutral-50 border border-neutral-200 rounded-sm overflow-hidden hover:shadow-md transition-shadow duration-300">
                ...
                <div className="md:col-span-7 flex flex-col p-5 md:p-6">
                  ...
                  <h3 className="text-[17px] font-medium text-luxury-black leading-snug mb-4 uppercase group-hover:text-luxury-black/75 transition-colors">{s.name}</h3>
                  <div className="flex items-center gap-7 mb-4">
                    <div className="text-center"><p className="text-[12px] tracking-[0.1em] uppercase text-luxury-black/50 mb-0.5">Beds</p><p className="text-[16px] text-luxury-black font-light">{s.beds}</p></div>
                    <div className="text-center"><p className="text-[12px] tracking-[0.1em] uppercase text-luxury-black/50 mb-0.5">Baths</p><p className="text-[16px] text-luxury-black font-light">{s.baths}</p></div>
                    <div className="text-center"><p className="text-[12px] tracking-[0.1em] uppercase text-luxury-black/50 mb-0.5">Built</p><p className="text-[16px] text-luxury-black font-light">{s.sqm} m²</p></div>
                  </div>
      ...
      {lightbox !== null && (
        <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center" onClick={() => setLightbox(null)}>
          ...
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-1.5" onClick={(e) => e.stopPropagation()}>
            {p.images.map((img, i) => (
              <button key={i} onClick={() => setLightbox(i)} className={`w-[56px] h-[40px] overflow-hidden transition-all ${i === lightbox ? "ring-2 ring-white opacity-100" : "opacity-40 hover:opacity-70"}`}>
