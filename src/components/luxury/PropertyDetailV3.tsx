...
const SIMILAR = [
  { image: prop1, name: "Beachfront Villa Es Cubells", location: "Es Cubells, Ibiza", price: "€6,200,000", beds: 6, baths: 5, sqm: 580, tag: "FOR SALE", href: "/property3/3001" },
  { image: prop2, name: "Modern Penthouse Marina Botafoch", location: "Marina Botafoch, Ibiza", price: "€3,100,000", beds: 3, baths: 3, sqm: 210, tag: "FOR SALE", href: "/property3/3002" },
  { image: prop3, name: "Traditional Finca San Carlos", location: "San Carlos, Ibiza", price: "€5,800,000", beds: 7, baths: 6, sqm: 750, tag: "FOR SALE", href: "/property3/3003" },
];

/* ─── Component ─── */
const PropertyDetailV3 = () => {
  ...
  const handleChatSend = () => {
    ...
    setChatInput("");
    setTimeout(() => { setChatMessages(prev => [...prev, { role: "bot", text: `Thank you for your interest in this property in ${p.location}. An advisor will follow up shortly.` }]); }, 1000);
  };
    ...
    <div className="flex-1 bg-white text-luxury-black font-sans">
        ...
        <div className="lg:hidden fixed inset-0 z-[100] bg-white flex flex-col">
          <div className="flex items-center justify-between px-4 h-[60px] border-b border-neutral-100 shrink-0">
            ...
            <Link to="/" className="flex flex-col items-center" onClick={() => setMobileMenuOpen(false)}>
              <span className="text-base tracking-[0.3em] font-light">{brand.fullName}</span>
            </Link>
            <div className="w-6" />
          </div>
          ...
          <div className="px-10 py-6 border-t border-neutral-100 shrink-0">
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {languages.map(lang => (
                <button key={lang.code} onClick={() => setCurrentLang(lang.code)} className={`flex items-center gap-1.5 px-3 py-2 text-[12px] rounded-sm border transition-colors ${currentLang === lang.code ? "border-luxury-black/30 bg-neutral-50 font-medium" : "border-neutral-200 font-light text-luxury-black/55"}`}>
      ...
      <section>
        ...
        <div className="lg:hidden flex items-center justify-between px-4 py-3 border-b border-neutral-100">
          ...
          <div className="flex items-center gap-3.5">
            <button className="text-luxury-black/50" onClick={() => {}}><Share2 className="w-[18px] h-[18px]" /></button>
        ...
        <div className="hidden lg:grid grid-cols-4 grid-rows-2 gap-1.5 h-[540px]">
          ...
          <div className="relative overflow-hidden cursor-pointer group" onClick={() => setLightbox(3)}>
            <img src={p.images[3]} alt="" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
            <div className="absolute bottom-3 left-3 flex items-center gap-2">
              {p.hasVideo && <button onClick={e => e.stopPropagation()} className="flex items-center gap-1.5 bg-white/90 backdrop-blur-sm text-luxury-black text-[12px] font-medium px-3 py-2 rounded-full shadow-sm"><Play className="w-3.5 h-3.5" /> Video</button>}
            </div>
          </div>
      ...
      <main className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 py-5 lg:py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-8">
          <div className="lg:col-span-7">
            <div className="flex items-center gap-2 text-[13px] text-luxury-black/50 font-light mb-3">
              <Link to="/" className="hover:text-luxury-black">Home</Link>
              <ChevronRight className="w-3 h-3 text-luxury-black/30" />
              <Link to="/properties" className="hover:text-luxury-black">Properties</Link>
              <ChevronRight className="w-3 h-3 text-luxury-black/30" />
              <span className="text-luxury-black/70 font-normal">{p.breadcrumb[p.breadcrumb.length - 1]}</span>
            </div>
            <p className="text-[13px] tracking-[0.14em] uppercase text-luxury-black/60 mb-1">{p.location}</p>
            <p className="text-[13px] text-luxury-black/50 font-light mb-2">Detached houses <span className="mx-1 text-luxury-black/25">|</span> <span className="italic">{p.style}</span> <span className="mx-1 text-luxury-black/25">|</span> <span className="font-mono text-luxury-black/40 text-[12px]">REF-{p.ref}</span></p>
            <h1 className="text-[20px] sm:text-[22px] md:text-[26px] lg:text-[30px] font-medium text-luxury-black leading-snug uppercase tracking-[0.01em] mb-2">{p.title}</h1>
            <p className="text-[14px] text-luxury-black/55 font-light leading-relaxed">{p.subtitle}</p>
            {/* Inline price on mobile */}
            <div className="lg:hidden flex flex-wrap items-baseline gap-x-3 gap-y-1 mt-3 mb-2">
            ...
            <div className="flex flex-wrap items-center gap-4 sm:gap-6 mt-4">
              {[
                ...
                <div key={i} className="flex items-center gap-2 text-[13px] sm:text-[14px] text-luxury-black/70">
                  <s.icon className="w-4 h-4 text-luxury-black/35" strokeWidth={1.5} />
                  <span className="font-light">{s.value}</span>
                  <span className="text-[10px] sm:text-[11px] text-luxury-black/40 uppercase tracking-wider">{s.label}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="hidden lg:block lg:col-span-5">
            <div className="bg-neutral-50 border border-neutral-200 rounded-sm p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <Popover>
                  <PopoverTrigger asChild><button className="w-8 h-8 flex items-center justify-center rounded-full border border-neutral-200 text-luxury-black/40 hover:text-luxury-black transition-all"><FileDown className="w-3.5 h-3.5" /></button></PopoverTrigger>
              ...
              {p.alsoForRent && <p className="text-[13px] text-luxury-black/45 mb-5 flex items-center gap-1.5"><Home className="w-3.5 h-3.5 text-luxury-gold/60" /> Also for rent: <span className="font-medium text-luxury-black/65">{p.rentalPrice}</span></p>}
              <div className="flex gap-2 mb-2">
                <a href={`tel:${p.agency.phone}`} className="flex-1 flex items-center justify-center gap-2 bg-luxury-black text-white text-[12px] tracking-[0.1em] uppercase py-3 hover:bg-luxury-black/85 transition-all"><Phone className="w-3.5 h-3.5" /> Call</a>
                <a href={`https://wa.me/${p.agency.whatsapp}`} target="_blank" rel="noopener noreferrer" className="flex-1 flex items-center justify-center gap-2 bg-[#25D366] text-white text-[12px] tracking-[0.1em] uppercase py-3 hover:bg-[#22bf5b] transition-all"><MessageCircle className="w-3.5 h-3.5" /> WhatsApp</a>
              </div>
              <button onClick={() => setEnquiryOpen(true)} className="w-full flex items-center justify-center gap-2 border border-neutral-300 text-luxury-black text-[12px] tracking-[0.1em] uppercase py-3 hover:bg-neutral-100 transition-all"><Mail className="w-3.5 h-3.5" /> Send Enquiry</button>
            </div>
          </div>
        </div>
      ...
      <section className="border-t border-neutral-200">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 py-10">
          <h2 className="text-xl font-light text-luxury-black tracking-tight mb-8">Similar Properties</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {SIMILAR.map((s, i) => (
              <Link key={i} to={s.href} className="group bg-neutral-50 border border-neutral-200 rounded-sm overflow-hidden hover:shadow-md transition-shadow">
                <div className="relative overflow-hidden aspect-[4/3]"><img src={s.image} alt={s.name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" /><span className="absolute top-3 left-3 text-[11px] tracking-[0.15em] uppercase bg-white/90 backdrop-blur-sm text-luxury-black px-2.5 py-1 font-medium">{s.tag}</span></div>
                <div className="p-5">
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 py-12">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
            <div>
              ...
              <p className="text-[13px] text-luxury-black/55 font-light mt-2 leading-relaxed">Receive our top luxury picks delivered to your inbox each week.</p>
            </div>
            <form className="flex flex-col gap-3" onSubmit={e => e.preventDefault()}>
              ...
              <button type="submit" className="bg-luxury-black text-white text-[13px] tracking-[0.1em] uppercase py-3 w-full hover:bg-luxury-black/85 transition-all">Subscribe to Newsletter</button>
            </form>
          </div>
        </div>
      </section>
        ...
        <DialogContent overlayClassName="!z-[110]" className="max-w-lg p-0 rounded-md border-2 border-neutral-300 overflow-hidden shadow-xl !z-[110]">
              ...
              <form className="p-6 pt-2 space-y-3" onSubmit={e => { e.preventDefault(); setEnquirySent("thanks"); setTimeout(() => setEnquirySent("suggestions"), 5000); }}>
                ...
                {wantVisit && (
                  <div className="space-y-2 bg-neutral-50 border border-neutral-200 p-3 rounded-sm">
                    ...
                    <select value={visitTime} onChange={e => setVisitTime(e.target.value)} className="w-full border border-neutral-300 px-4 py-2.5 text-[14px] appearance-none cursor-pointer focus:outline-none rounded-sm bg-white">
                ...
                <button type="submit" className="w-full bg-luxury-black text-white text-[13px] tracking-[0.1em] uppercase py-3 hover:bg-luxury-black/85 transition-all">{wantVisit ? "Request Visit" : "Send Enquiry"}</button>
              </form>
            </>
          )}
          {(enquirySent === "thanks" || enquirySent === "suggestions") && (
            <div className="p-6 max-h-[80vh] overflow-y-auto">
              ...
              <div className="text-center py-6"><div className="w-14 h-14 rounded-full bg-luxury-black/5 flex items-center justify-center mx-auto mb-4"><Check className="w-7 h-7 text-luxury-black" /></div><h3 className="text-[18px] font-medium text-luxury-black mb-2">Thank You</h3><p className="text-[14px] text-luxury-black/55 font-light">We'll be in touch shortly about this property.</p></div>
              {enquirySent === "suggestions" && (
                <>
                  <p className="text-[12px] tracking-[0.1em] uppercase text-luxury-black/40 font-medium mb-3 mt-4">You may also like</p>
                  <div className="space-y-2">
                    {SIMILAR.map((s, i) => (
                      <Link key={i} to={s.href} onClick={() => { setEnquiryOpen(false); setEnquirySent("idle"); }} className="flex gap-3 bg-neutral-50 border border-neutral-200 rounded-sm overflow-hidden hover:bg-neutral-100 transition-all">
      ...
      <Dialog open={langOpen} onOpenChange={setLangOpen}>
        <DialogContent className="max-w-md p-6 rounded-md border-2 border-neutral-300 shadow-xl">
          ...
          <div className="grid grid-cols-3 gap-2">
            {languages.map(lang => (
              <button key={lang.code} onClick={() => { setCurrentLang(lang.code); setLangOpen(false); }} className={`flex flex-col items-center gap-2 px-3 py-4 rounded-sm text-[13px] border transition-colors ${currentLang === lang.code ? "bg-neutral-50 border-neutral-300 font-medium" : "border-transparent text-luxury-black/55 font-light hover:bg-neutral-50"}`}>
      ...
      {/* Chatbot */}
      {!chatOpen && <button onClick={() => setChatOpen(true)} className="fixed z-50 w-14 h-14 rounded-full bg-luxury-black text-white shadow-lg flex items-center justify-center hover:bg-luxury-black/85 transition-all bottom-[72px] right-4 lg:bottom-6 lg:right-6"><MessageCircle className="w-5 h-5" /></button>}
      {chatOpen && (
        <div className="fixed z-50 bg-white border border-neutral-200 shadow-xl flex flex-col inset-0 lg:inset-auto lg:bottom-6 lg:right-6 lg:w-[380px] lg:h-[520px] lg:rounded-lg lg:overflow-hidden">
          ...
          <div className="flex items-center gap-3 px-4 py-3 border-b border-neutral-100 bg-neutral-50/50">
            ...
            <div className="min-w-0"><p className="text-[13px] font-medium truncate">Luxury Villa Ibiza</p><p className="text-[15px] font-light">{p.priceFormatted}</p></div>
          </div>
          ...
          <div className="border-t border-neutral-200 px-3 py-3 flex items-center gap-2">
            <input type="text" value={chatInput} onChange={e => setChatInput(e.target.value)} onKeyDown={e => e.key === "Enter" && handleChatSend()} placeholder="Ask about this property..." className="flex-1 text-[13px] placeholder:text-luxury-black/30 px-3 py-2 border border-neutral-200 rounded-full focus:outline-none focus:border-luxury-black/30" />
            <button onClick={handleChatSend} className="w-9 h-9 flex items-center justify-center rounded-full bg-luxury-black text-white"><Send className="w-4 h-4" /></button>
          </div>
        </div>
      )}
...
export default PropertyDetailV3;
