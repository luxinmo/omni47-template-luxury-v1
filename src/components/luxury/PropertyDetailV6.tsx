        ...
        <div className="max-w-[1400px] mx-auto grid grid-cols-3 items-center px-6 lg:px-10 h-[68px]">
          <div className="hidden lg:flex items-center gap-10">
            <div className="relative">
              <button onClick={() => setLangOpen(!langOpen)} className="flex items-center gap-1.5 text-luxury-black/50 hover:text-luxury-black transition-colors duration-300" aria-label="Select language">
              ...
              {langOpen && (
                <>
                  ...
                  <div className="absolute top-full left-1/2 -translate-x-1/2 mt-3 z-50 bg-white shadow-xl rounded-md py-2 min-w-[180px] border border-neutral-200">
                    <p className="text-[10px] tracking-[0.15em] uppercase px-4 py-1.5 font-medium text-luxury-black/40">Language</p>
                    {languages.map((lang) => (
                      <button
                        key={lang.code}
                        onClick={() => { setCurrentLang(lang.code); setLangOpen(false); }}
                        className={`w-full flex items-center gap-3 px-4 py-2.5 text-[13px] hover:bg-neutral-50 transition-colors ${currentLang === lang.code ? "text-luxury-black font-medium" : "text-luxury-black/55 font-light"}`}
                      >
                        <span className="text-lg leading-none">{lang.flag}</span>
                        <span>{lang.label}</span>
                        {currentLang === lang.code && (
                          <svg className="w-3.5 h-3.5 ml-auto opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>
                        )}
                      </button>
                    ))}
                  </div>
