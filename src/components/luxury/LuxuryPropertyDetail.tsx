    ...
    <div className="flex-1 overflow-auto bg-white text-luxury-black font-sans">
      ...
      <nav className="sticky top-0 z-50 bg-luxury-black">
        <div className="max-w-[1400px] mx-auto flex items-center justify-between px-6 lg:px-10 h-[56px]">
          <a href="/" className="font-serif text-base tracking-[0.25em] text-white font-light">{BRAND_NAME}</a>
          <div className="hidden md:flex items-center gap-8">
            {["Properties", "Rentals", "About", "Contact"].map((l) => (
              <a key={l} href="#" className="text-[11px] tracking-[0.18em] uppercase text-white/60 hover:text-white transition-colors duration-300">{l}</a>
            ))}
          </div>
            ...
            <div key={i} className="hidden md:block relative overflow-hidden cursor-pointer group" onClick={() => openLightbox(i + 1)}>
              ...
              {i === 3 && PROPERTY.images.length > 5 && (
                <div className="absolute inset-0 bg-luxury-black/50 flex items-center justify-center">
                  <span className="text-white text-[13px] tracking-wide font-light">+{PROPERTY.images.length - 5} photos</span>
                </div>
              )}
            </div>
          ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="border border-neutral-200 p-6">
              ...
              <a href={`mailto:${PROPERTY.agent.email}`} className="flex items-center justify-center gap-2 border border-luxury-black/20 text-luxury-black/70 text-[11px] tracking-[0.15em] uppercase py-3 w-full hover:bg-luxury-black hover:text-white transition-all duration-300">
      ...
      <section className="max-w-[1400px] mx-auto px-6 lg:px-10 py-10 border-t border-neutral-100">
        <h2 className="text-xl font-light text-luxury-black font-serif tracking-tight mb-6">Similar Properties Nearby</h2>
      ...
      <section className="py-14 bg-neutral-50/60">
        <div className="max-w-xl mx-auto px-6 text-center">
          ...
          <form className="flex flex-col sm:flex-row gap-3" onSubmit={(e) => e.preventDefault()}>
            <input type="email" placeholder="Your email address" className="flex-1 border border-neutral-300 bg-white px-4 py-3 text-[13px] text-luxury-black placeholder:text-luxury-black/30 focus:outline-none focus:border-luxury-black/40 transition-colors duration-300" />
            <button type="submit" className="bg-luxury-black text-white text-[11px] tracking-[0.15em] uppercase px-8 py-3 hover:bg-luxury-black/85 transition-all duration-300 whitespace-nowrap">Subscribe</button>
          </form>
        </div>
      </section>
      ...
      {lightbox !== null && (
        <div className="fixed inset-0 z-[100] bg-luxury-black/95 flex items-center justify-center" onClick={closeLightbox}>
          ...
          />
          <p className="absolute bottom-5 text-white/30 text-[12px] font-light">{lightbox + 1} / {PROPERTY.images.length}</p>
        </div>
      )}
    </div>
  );
