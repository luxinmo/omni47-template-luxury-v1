...
const PropertyDetailV8 = () => {
  ...
  const handleChatSend = () => {
    ...
    setChatInput("");
    setTimeout(() => {
      setChatMessages((prev) => [...prev, { role: "bot", text: `Thank you for your interest in this property in ${p.location}. An advisor will follow up shortly.` }]);
    }, 1000);
  };
  ...
  return (
    <div className="flex-1 bg-white text-luxury-black font-sans">
      ...
      {/* ═══ NAVBAR ═══ */}
      <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-neutral-100" aria-label="Main navigation">
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden fixed inset-0 z-[100] bg-white flex flex-col">
          <div className="flex items-center justify-between px-4 md:px-6 h-[60px] md:h-[68px] border-b border-neutral-100 shrink-0">
            <button onClick={() => setMobileMenuOpen(false)} aria-label="Close menu"><X className="w-6 h-6" /></button>
            <Link to="/" className="flex flex-col items-center" onClick={() => setMobileMenuOpen(false)}>
              <span className="text-base md:text-lg tracking-[0.3em] font-light">{brand.fullName}</span>
              <span className="text-[9px] md:text-[10px] tracking-[0.35em] uppercase font-light text-luxury-black/40">{brand.subtitle}</span>
            </Link>
            <div className="w-6" />
          </div>
          <div className="flex-1 flex flex-col justify-center px-10">
            {[...navLeft, ...navRight].map((item) => (
              <Link key={item.label} to={item.href} onClick={() => setMobileMenuOpen(false)} className="text-[18px] tracking-[0.15em] uppercase font-light py-4 text-luxury-black/80 border-b border-neutral-100 last:border-b-0 hover:text-luxury-black transition-colors text-center">
                {item.label}
              </Link>
            ))}
          </div>
          <div className="px-10 py-6 border-t border-neutral-100 shrink-0">
            <div className="flex flex-wrap justify-center gap-2 mb-4">
              {languages.map((lang) => (
                <button key={lang.code} onClick={() => setCurrentLang(lang.code)} className={cn("flex items-center gap-1.5 px-3 py-2 text-[12px] tracking-[0.05em] border transition-colors", currentLang === lang.code ? "border-luxury-black/30 bg-neutral-50 font-medium" : "border-neutral-200 font-light text-luxury-black/55 hover:bg-neutral-50")}>
      ...
      <section aria-label="Property photos">
        ...
        <div className="lg:hidden flex items-center justify-between px-4 py-3 border-b border-neutral-100">
          <div className="flex items-center gap-3.5">
            {p.hasVideo && (
              <button className="flex items-center gap-1.5 text-[13px] text-luxury-black/70 font-medium"><Play className="w-4 h-4" /> Video</button>
            )}
          ...
          <div className="flex items-center gap-3.5">
            <button className="text-luxury-black/50 hover:text-luxury-black transition-colors"><Share2 className="w-[18px] h-[18px]" /></button>
            <button onClick={() => setLiked(!liked)} className={cn("transition-colors", liked ? "text-luxury-black" : "text-luxury-black/50 hover:text-luxury-black")}>
        ...
        <div className="hidden lg:grid grid-cols-3 gap-1 h-[600px]">
          ...
          <div className="col-span-2 row-span-1 relative overflow-hidden cursor-pointer group" onClick={() => setLightbox(0)}>
            ...
            <div className="absolute top-4 right-4 flex items-center gap-2">
              ...
              <button onClick={(e) => { e.stopPropagation(); setLiked(!liked); }} className={cn("flex items-center gap-1.5 text-[12px] font-medium px-3.5 py-2 rounded-full shadow-sm transition-all", liked ? "bg-luxury-black text-white" : "bg-white/90 backdrop-blur-sm text-luxury-black hover:bg-white")}>
      ...
      <section className="max-w-[1400px] mx-auto px-6 lg:px-10 py-5">
        <div className="border border-neutral-200 rounded-sm px-8 py-8 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <button className="shrink-0 border border-neutral-300 text-luxury-black text-[11px] tracking-[0.12em] uppercase px-6 py-3 hover:bg-luxury-black hover:text-white transition-all duration-300 rounded-sm">Download Guide</button>
        </div>
      </section>
      ...
      <section className="border-t border-neutral-200">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10 py-10">
          <h2 className="text-xl font-light text-luxury-black tracking-tight mb-5">About Santa Eulalia del Río</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="text-[14px] leading-[1.9] text-luxury-black/70 font-light space-y-4">
              ...
              <p>The area remains one of the most desirable locations in the Balearic Islands for both primary residences and investment properties.</p>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-neutral-50 border border-neutral-200 h-[200px] flex items-center justify-center text-luxury-black/30 text-[13px] font-light rounded-sm">
                <MapPin className="w-5 h-5 mr-1.5" /> Map
              </div>
              <div className="bg-neutral-50 border border-neutral-200 h-[200px] flex flex-col items-center justify-center text-luxury-black/30 text-[13px] font-light rounded-sm gap-2">
                <Play className="w-6 h-6" strokeWidth={1} /> Area Video
              </div>
            </div>
          </div>
        </div>
            ...
            {SIMILAR.map((s, i) => (
              <Link key={i} to={s.href} className="group bg-neutral-50 border border-neutral-200 rounded-sm overflow-hidden hover:shadow-md transition-shadow duration-300">
                ...
                <div className="p-5">
                  ...
                  <h3 className="text-[15px] font-medium text-luxury-black mb-2 group-hover:text-luxury-black/75 transition-colors">{s.name}</h3>
                  <div className="flex items-center gap-5 mb-3 text-[13px] text-luxury-black/60 font-light">
      ...
      <section className="border-t border-neutral-200 bg-neutral-50">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10 py-8">
          <h2 className="text-lg font-light text-luxury-black tracking-tight mb-4">Explore Properties in Ibiza</h2>
          <div className="flex flex-wrap gap-3">
            {INTERNAL_LINKS.map((link, i) => (
              <Link key={i} to={link.href} className="text-[13px] text-luxury-black/65 font-light border border-neutral-200 bg-white rounded-full px-5 py-2.5 hover:bg-luxury-black hover:text-white hover:border-luxury-black transition-all duration-300">{link.label}</Link>
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="border-t border-neutral-200">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-10 py-6">
          <p className="text-[14px] font-normal text-luxury-black mb-3">Recently Viewed</p>
          <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
            {[...SIMILAR, ...SIMILAR].slice(0, 5).map((s, i) => (
              <Link key={i} to={s.href} className="shrink-0 w-[150px] group">
                <div className="overflow-hidden aspect-[4/3] mb-1.5">
            ...
            <>
              ...
              <form className="p-6 pt-2 space-y-3" onSubmit={(e) => { e.preventDefault(); setEnquirySent("thanks"); setTimeout(() => setEnquirySent("suggestions"), 5000); }}>
                ...
                <label className="flex items-start gap-2 cursor-pointer">
                  <input type="checkbox" defaultChecked className="w-4 h-4 border-neutral-300 mt-0.5 accent-luxury-black" />
                  <span className="text-[11px] text-luxury-black/45 font-light leading-relaxed">
                ...
                <button type="submit" className="w-full bg-luxury-black text-white text-[13px] tracking-[0.12em] uppercase py-3.5 hover:bg-luxury-black/85 transition-all mt-2">Send Enquiry</button>
              </form>
            </>
          )}
          {enquirySent === "thanks" && (
            <div className="p-8 text-center">
              ...
              <DialogTitle className="text-[20px] font-medium text-luxury-black mb-2">Thank You</DialogTitle>
          ...
          {enquirySent === "suggestions" && (
            <div className="p-5">
              ...
              <div className="space-y-2.5">
                {SIMILAR.map((s, i) => (
                  <Link key={i} to={s.href} onClick={() => { setEnquiryOpen(false); setEnquirySent("idle"); }} className="flex gap-3 bg-neutral-50 border border-neutral-200 rounded-sm overflow-hidden hover:bg-neutral-100 hover:border-neutral-300 transition-all group">
                    ...
                    <div className="py-2 pr-3 flex flex-col justify-center min-w-0">
                      ...
                      <p className="text-[13px] font-medium text-luxury-black line-clamp-1 group-hover:text-luxury-black/75 transition-colors">{s.name}</p>
      ...
      <Dialog open={langOpen} onOpenChange={setLangOpen}>
        <DialogContent className="max-w-md p-6 rounded-md border-2 border-neutral-300 overflow-hidden shadow-xl">
          ...
          <div className="grid grid-cols-3 gap-2">
            {languages.map((lang) => (
              <button key={lang.code} onClick={() => { setCurrentLang(lang.code); setLangOpen(false); }} className={cn("flex flex-col items-center gap-2 px-3 py-4 rounded-sm text-[13px] border transition-colors", currentLang === lang.code ? "bg-neutral-50 border-neutral-300 text-luxury-black font-medium" : "border-transparent text-luxury-black/55 font-light hover:bg-neutral-50")}>
      ...
      {chatOpen && (
        <div className="fixed z-50 bg-white border border-neutral-200 shadow-xl flex flex-col inset-0 lg:inset-auto lg:bottom-6 lg:right-6 lg:w-[380px] lg:h-[520px] lg:rounded-lg lg:overflow-hidden">
          ...
          <div className="border-t border-neutral-200 px-3 py-3 flex items-center gap-2">
            <input type="text" value={chatInput} onChange={(e) => setChatInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleChatSend()} placeholder="Ask about this property..." className="flex-1 text-[13px] text-luxury-black placeholder:text-luxury-black/30 px-3 py-2 border border-neutral-200 rounded-full focus:outline-none focus:border-luxury-black/30 transition-colors" />
            <button onClick={handleChatSend} className="w-9 h-9 flex items-center justify-center rounded-full bg-luxury-black text-white hover:bg-luxury-black/85 transition-colors shrink-0"><Send className="w-4 h-4" /></button>
          </div>
        </div>
      )}
          ...
          <div className="flex-1 relative flex items-center justify-center min-h-0" onTouchStart={handleLbTouchStart} onTouchEnd={handleLbTouchEnd}>
            {lightbox < p.images.length ? (
              <>
                ...
                {lightbox > 0 && <button onClick={() => setLightbox(lightbox - 1)} className="hidden lg:flex absolute left-4 z-20 text-white/30 hover:text-white transition-colors"><ChevronLeft className="w-8 h-8" strokeWidth={1} /></button>}
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                ...
                <div className="relative z-10 text-center px-6 max-w-lg">
                  ...
                  <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
                    <a href={`tel:${p.agency.phone}`} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-white text-luxury-black text-[12px] tracking-[0.1em] uppercase px-8 py-3 hover:bg-white/90 transition-all"><Phone className="w-3.5 h-3.5" /> Call</a>
                    <a href={`https://wa.me/${p.agency.whatsapp}`} target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto flex items-center justify-center gap-2 bg-[#25D366] text-white text-[12px] tracking-[0.1em] uppercase px-8 py-3 hover:bg-[#22bf5b] transition-all"><MessageCircle className="w-3.5 h-3.5" /> WhatsApp</a>
                    <button onClick={() => setEnquiryOpen(true)} className="w-full sm:w-auto flex items-center justify-center gap-2 border border-white/30 text-white text-[12px] tracking-[0.1em] uppercase px-8 py-3 hover:bg-white/10 transition-all"><Mail className="w-3.5 h-3.5" /> Enquiry</button>
                  </div>
                  <button onClick={() => setLightbox(lightbox - 1)} className="mt-8 text-white/30 hover:text-white/60 text-[12px] tracking-[0.1em] uppercase transition-colors flex items-center gap-1 mx-auto"><ChevronLeft className="w-3.5 h-3.5" /> Back to photos</button>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="fixed inset-0 z-[100] bg-black/95 flex flex-col overflow-y-auto" role="dialog">
          ...
          <div className="px-4 sm:px-8 py-10 text-center shrink-0">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 max-w-md mx-auto">
              <a href={`tel:${p.agency.phone}`} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-white text-luxury-black text-[12px] tracking-[0.1em] uppercase px-8 py-3 hover:bg-white/90 transition-all"><Phone className="w-3.5 h-3.5" /> Call</a>
              <a href={`https://wa.me/${p.agency.whatsapp}`} target="_blank" rel="noopener noreferrer" className="w-full sm:w-auto flex items-center justify-center gap-2 bg-[#25D366] text-white text-[12px] tracking-[0.1em] uppercase px-8 py-3 hover:bg-[#22bf5b] transition-all"><MessageCircle className="w-3.5 h-3.5" /> WhatsApp</a>
              <button onClick={() => { setGridView(false); setEnquiryOpen(true); }} className="w-full sm:w-auto flex items-center justify-center gap-2 border border-white/30 text-white text-[12px] tracking-[0.1em] uppercase px-8 py-3 hover:bg-white/10 transition-all"><Mail className="w-3.5 h-3.5" /> Enquiry</button>
            </div>
          </div>
        </div>
      )}
